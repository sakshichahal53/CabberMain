...
import com.example.sakshi.cabber.fragments.ReferralFragment;
...
public class WhereToGoActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleApiClient.OnConnectionFailedListener,
    ...
    private void showRefferalDialog() {
        getSupportFragmentManager().beginTransaction().add(R.id.mainFrame, new ReferralFragment(), ReferralFragment.class.getName()).addToBackStack(ReferralFragment.class.getName()).commit();
    }

    @Override
    public void onBackPressed() {
        if (getSupportFragmentManager().findFragmentByTag(ReferralFragment.class.getName()) == null)
            super.onBackPressed();
    }
